        ...
        public override void _Process(float delta)
        {
            ...
            if (isMoving && !Croisement) //Si le vehicule est en mouvement et que l'on n'est pas a un croisement
            {
                    ...
                    (direction == Direction.BOTTOM && Position <= arrive)) //Si le vehicule est arrive au bloc suivant
                {
                    Vector2 positionActuel = _planInitial.TileMap2.WorldToMap(Position);
                    Vector2
                        NextCase = DirectionToVector2(direction) +
                                   new Vector2(-1,
                                       -1); // Position du bloc suivant. new Vector2(-1, -1) pour difference dans le TileSet
                    if (_planInitial.GetBlock(_planInitial.TileMap1, (int) positionActuel.x + (int) NextCase.x,
                            (int) positionActuel.y + (int) NextCase.y) ==
                        -1) //Enleve le vehicule quand il sort de la map
                    {
                        PlanInitial.NbCar -= 1;
                        this.QueueFree();
                    }
                    ...
                    if (!isMovingCroisment && Routes.IsRoute(_planInitial.GetBlock(_planInitial.TileMap2,
                                               (int) positionActuel.x + (int) NextCase.x,
                                               (int) positionActuel.y + (int) NextCase.y))
                                           ...
                                           && !Routes.IsVirage(_planInitial.GetBlock(_planInitial.TileMap2,
                                               (int) positionActuel.x + (int) NextCase.x,
                                               (int) positionActuel.y + (int) NextCase.y))
                    ) //Si le bloc suivant est une route mais pas un croisment ni un virage
                    {
                    ...
                    else if (isMovingCroisment) // Si le vehicule réalise un deplacement croisement
                    {
                        ...
                        Decallage = DecallageDico[_animatedSprite.Animation];
                             ...
                             && !Routes.IsVirage(_planInitial.GetBlock(_planInitial.TileMap2,
                                 (int) positionActuel.x + (int) NextCase.x, (int) positionActuel.y + (int) NextCase.y))
                    ) //Si le bloc suivant est un croisment mais pas un virage
                    {
                        Croisement = true;
                        isMoving = false;
                        _deplacement = new Vector2(0, 0);
                        BlocCroisment = new Vector2((int) positionActuel.x + (int) NextCase.x,
                            (int) positionActuel.y + (int) NextCase.y);
                    }
                    else if (Routes.IsVirage(_planInitial.GetBlock(_planInitial.TileMap2,
                            (int) positionActuel.x + (int) NextCase.x, (int) positionActuel.y + (int) NextCase.y)))
