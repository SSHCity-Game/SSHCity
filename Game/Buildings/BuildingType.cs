using SshCity.Game.Plan;

namespace SshCity.Game.Buildings
{
    /// <summary>
    /// Contient les différents type de bâtiments présent dans le jeu, ainsi que leurs ids respectifs pour les textures dans godot
    /// </summary>
    public enum BuildingType
    {
        CAFE = Ref_donnees.cafe,
        CASERNE = Ref_donnees.caserne,
        CENTRALE = Ref_donnees.centrale,
        EGLISE = Ref_donnees.eglise,
        FERME = Ref_donnees.ferme,
        HOSPITAL = Ref_donnees.hopital,
        HOTEL = Ref_donnees.hotel,
        IMMEUBLE = Ref_donnees.immeuble_brique,
        IMMEUBLE_VERT = Ref_donnees.immeuble_vert,
        MAISON = Ref_donnees.maison1,
        MAISON3 = Ref_donnees.maison3,
        MAISON4 = Ref_donnees.maison4,
        MAISON5 = Ref_donnees.maison5,
        MCALLY = Ref_donnees.McAffy,
        PARC = Ref_donnees.parc_enfant,
        PISCINE = Ref_donnees.piscine,
        POLICE = Ref_donnees.police,
        RESTAURANT = Ref_donnees.restaurant,
        RESTAURANT2 = Ref_donnees.restaurant2,
        MAIRIE = Ref_donnees.mairie,
        EPURATION = Ref_donnees.stationEpuration,
        MOSQUE = Ref_donnees.mosque,
        FERMEGRANDE = Ref_donnees.ferme3,
        EOLIENNE = Ref_donnees.eolienne,
    }
}