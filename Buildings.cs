...
public class Buildings : Node2D
{
    ...
    public override void _Ready()
    {
        ...
        for (int i = min_x; i < max_x; i++)
        {
            ...
            if (acc > 0)
            {
                if (_tileMap1.GetCell(i, y2 - 1) == 0)
                {
                    _tileMap1.SetCell(i, y2, 4);
                    _tileMap1.SetCell(i, y2 - 1, 4);
                    acc -= 1;
                    y2 -= 1;
                    continuer = false;
                }
        ...
        for (int i = min_y; i < max_y; i++)
        {
            ...
            if (acc2 > 0)
            {
                if (_tileMap1.GetCell(x2 - 1, i) == 0)
                {
                    _tileMap1.SetCell(x2, i, 4);
                    _tileMap1.SetCell(x2 - 1, i, 4);
                    acc2 -= 1;
                    x2 -= 1;
                    continuer2 = false;
                }
            ...
            if (avant2(x2, i) && (_tileMap1.GetCell(x2-1, i-1) == 4))
            {
                x2 = refer - 1;
                refer -= 1;
            }
        ...
        private bool avant(int x, int y)
         {
             return _tileMap1.GetCell(x , y) == 6 || _tileMap2.GetCell(x, y) == indexe_montagne;
         }
         private bool gauche(int x, int y)
         {
             return _tileMap1.GetCell(x , y-1) == 6 || _tileMap2.GetCell(x, y-1) == indexe_montagne;
         }
         ...
         {
             return _tileMap1.GetCell(x-1 , y) == 6 || _tileMap2.GetCell(x-1, y) == indexe_montagne;
         }
