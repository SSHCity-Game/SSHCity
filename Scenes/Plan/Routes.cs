...
namespace SshCity.Scenes.Plan
{
    public class Routes
    {
        ...
        public static int ChoixRoute(Vector2 tile, PlanInitial planInitial)
        {
            int x = (int)tile.x;
            int y = (int) tile.y;
            bool HD = IsRoute(planInitial.GetBlock(planInitial.TileMap2, x, y - 1)) && planInitial.GetBlock(planInitial.TileMap1, x+1, y) == Ref_donnees.route;
            bool BD = IsRoute(planInitial.GetBlock(planInitial.TileMap2, x + 1, y)) && planInitial.GetBlock(planInitial.TileMap1, x+2, y+1) == Ref_donnees.route;
            bool BG = IsRoute(planInitial.GetBlock(planInitial.TileMap2, x, y + 1)) && planInitial.GetBlock(planInitial.TileMap1, x+1, y+2) == Ref_donnees.route;
            bool HG = IsRoute(planInitial.GetBlock(planInitial.TileMap2, x - 1, y)) && planInitial.GetBlock(planInitial.TileMap1, x, y+1) == Ref_donnees.route;
